...
import type { Item, QuickActions, PaymentRow } from "@/lib/sales/salesTypes";
...
const useActiveTenantStore = () => ({ tenantId: "", storeId: "" });
const apiInventoryCheck = async (params: any) => ({ success: true });
const apiCheckout = async (params: any) => ({
  success: true,
  id: "temp",
  receipt_number: "TEMP-001",
});
...
const initialItems: Item[] = [
  {
    id: "1",
    name: "Орос цамц капштай, гоёл цамц",
    qty: 10,
    price: 200000,
    color: "Ногоон",
    size: "XL",
...
export default function SalesPage() {
  ...
  async function handlePaidMulti(
    rows: PaymentRow[],
    ...
    if (!tenantId || !storeId) {
      alert(
        "Сонгосон салбар эсвэл tenant байхгүй байна. (active-tenant-id / active-store-id)"
      );
      return;
    }
      ...
      <header className="flex items-center gap-2">
        ...
        {!tenantId || !storeId ? (
          <span className="text-xs text-red-600">
            ⚠️ Та эхлээд салбар/tenant сонгоно уу.
          </span>
        ) : busy ? (
        ...
        <div className="mt-2 mx-2.5 bg-white rounded-md border p-3 text-sm text-black space-y-1">
          ...
          {!!qa.discountPercent && (
            <div className="flex justify-between">
              ...
              <span>- {fmt(calcTotals(items, qa).discount)}</span>
            </div>
          )}
          {qa.includeVAT && (
            <div className="flex justify-between">
              <span>НӨАТ (10%)</span>
              <span>{fmt(calcTotals(items, qa).vat)}</span>
            </div>
          )}
          {!!qa.deliveryFee && (
            <div className="flex justify-between">
              <span>Хүргэлт</span>
              <span>{fmt(qa.deliveryFee)}</span>
            </div>
          )}
          <div className="flex justify-between font-semibold text-base">
            <span>Нийт төлөх</span>
            <span>{fmt(calcTotals(items, qa).grand)}</span>
          </div>
        </div>
      </main>
