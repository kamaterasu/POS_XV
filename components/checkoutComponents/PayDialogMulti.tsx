...
import type { PaymentRow } from '@/lib/sales/salesType';
...
export default function PayDialogMulti({
  ...
  onPaidMulti: (rows: PaymentRow[], totalReceived: number, change: number) => void;
}) {
  const [rows, setRows] = useState<PaymentRow[]>([{ method: 'cash', amount: 0 }]);
  ...
  const totalReceived = useMemo(() => rows.reduce((s, r) => s + (Number(r.amount) || 0), 0), [rows]);
  const change = Math.max(0, totalReceived - total);
  const remaining = Math.max(0, total - totalReceived);
  const canConfirm = totalReceived >= total && rows.length > 0 && rows.every(r => (r.amount ?? 0) >= 0);
  ...
  function removeRow(i: number) {
    setRows(prev => prev.length === 1 ? [{ method: 'cash', amount: total }] : prev.filter((_, idx) => idx !== i));
  }

  function handleConfirm() {
    if (!canConfirm) return;
    // normalize numbers
    const norm = rows.map(r => ({ ...r, amount: Math.round(Number(r.amount) || 0) }));
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-white text-black w-full max-w-md rounded-xl shadow-xl p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={onClose} className="text-black/60 hover:text-black text-sm">✕</button>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                setRows(prev => {
                  if (prev.length === 0) return [{ method: 'cash', amount: total }];
                  const copy = [...prev];
                  copy[copy.length - 1] = { ...copy[copy.length - 1], amount: (copy[copy.length - 1].amount || 0) + remaining };
                  return copy;
                });
              }}
        ...
        <div className="mt-3 border-t pt-3 text-sm space-y-1">
          <div className="flex justify-between"><span>Нийт төлөх</span><strong>{fmt(total)}</strong></div>
          <div className="flex justify-between"><span>Авагдсан</span><span>{fmt(totalReceived)}</span></div>
          <div className="flex justify-between"><span>Зөрүү</span><span className={remaining > 0 ? 'text-red-600' : ''}>{fmt(remaining)}</span></div>
          <div className="flex justify-between"><span>Хариулт</span><span>{fmt(change)}</span></div>
        </div>

        <div className="mt-3 flex gap-2 justify-end">
          <button className="h-9 px-4 border rounded bg-white hover:bg-[#fafafa]" onClick={onClose}>Болих</button>
          <button
            className={`h-9 px-4 rounded text-white ${canConfirm ? 'bg-[#5AA6FF] hover:opacity-90' : 'bg-[#b6d5ff] cursor-not-allowed'}`}
            disabled={!canConfirm}
